...
export async function symEncrypt(
  ...
  const encryptedData = await webcrypto.subtle.encrypt(
    {
      name: "AES-CBC",
      iv: new Uint8Array(12), // Pour un vrai usage, il faut générer un IV unique pour chaque chiffrement
    },
    key,
    encodedData
  );
...
export async function symDecrypt(
  ...
  const decryptedData = await webcrypto.subtle.decrypt(
    {
      name: "AES-GCM",
      iv: new Uint8Array(12), // Utiliser le même IV que pour le chiffrement
    },
    key,
    dataBuffer
  );
